                ...
                <tbody class="text-orange position-relative">
                    @foreach($riders as $rider)
                        ...
                        <tr class="text-orange\">
                            ...
                            <td class=""><a class="btn btn-sm btn-primary "
                                    href="{{ route('rider.profile', $rider) }}">Profilo</a></td>
                            <td class="w-100">
                                <a class="btn btn-sm btn-secondary me-2"
                                    href="{{ route('rider.edit', $rider) }}">modifica</a>
                                <button class="btn btn-sm btn-danger mt-2 mt-xxl-2  uppercased"
                                    data-action="{{ route('rider.destroy', $rider) }}" onclick="displayModaleRider()"
                                    id="delete-rider">Elimina</button>
                            </td>
                            <td><a class="btn btn-sm btn-warning mt-xxl-0"
                                    href="{{ route('delivery.rider.create', $rider)}}">Aggiungi Consegna</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
